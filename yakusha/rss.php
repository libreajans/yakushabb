...
$board_config['max_rss_topics'] = intval($board_config[Rss_count]);
//
...
// Get Various vars
//
$forum = isset($HTTP_GET_VARS[POST_FORUM_URL]) && intval($HTTP_GET_VARS[POST_FORUM_URL]) ? intval($HTTP_GET_VARS[POST_FORUM_URL]) : false;
$topic = isset($HTTP_GET_VARS[POST_TOPIC_URL]) && intval($HTTP_GET_VARS[POST_TOPIC_URL]) ? intval($HTTP_GET_VARS[POST_TOPIC_URL]) : false;
$post = isset($HTTP_GET_VARS[POST_POST_URL]) && intval($HTTP_GET_VARS[POST_POST_URL]) ? intval($HTTP_GET_VARS[POST_POST_URL]) : false;
...
if($post)
{
        $sql = "SELECT topic_id FROM ".POSTS_TABLE." WHERE post_id = ".intval($post);
...
// Start RSS output
//
$rss_result = "<?xml version=\"1.0\" encoding=\"$lang[ENCODING]\"?>
<rss version=\"2.0\">
<channel>
