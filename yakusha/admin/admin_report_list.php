		...
		if (!isset($_GET[POST_CAT_URL]) || $_GET[POST_CAT_URL] == $this_cat['cat_id'])
		{
			...
			else
			{
				...
				foreach ($this_cat['reports'] as $this_report)
				{
					...
					{
						$temp_url = append_sid($phpbb_root_path . "viewtopic.$phpEx?" . POST_POST_URL . '=' . $this_report['report_info'] . '#' . $this_report['report_info']);
					...
					{
						$temp_url = append_sid($phpbb_root_path . "viewtopic.$phpEx?" . POST_TOPIC_URL . '=' . $this_report['report_info']);
						$report_info = '<a href="' . $temp_url . '" class="genmed" target="_blank">' . $this_report['topic_title'] . '</a>';
					}
					else if ($this_cat['cat_id'] == REPORT_USER_ID && $this_report['username'])
					{
						$temp_url = append_sid($phpbb_root_path . "profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $this_report['report_info']);
					...
					$template->assign_block_vars('catrow.reportrow', array(
						...
						'U_USER' => append_sid($phpbb_root_path . "profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $this_report['report_user_id']),
						...
						'U_PRIVMSG' => append_sid($phpbb_root_path . "privmsg.$phpEx?mode=post&amp;" . POST_USERS_URL . '=' . $this_report['report_user_id']))
					);
