...
// favorites tablosundaki deðerlerini sil

   $sql = "DELETE FROM " . FAVORITES_TABLE. "
...
// üye hakkýndaki raporlarý da sil
   $sql = "DELETE FROM " . REPORT_TABLE. "
    WHERE report_info = $user_id";
   if( !$db->sql_query($sql) )
   {
    message_die(GENERAL_ERROR, 'Could not delete user', '', __LINE__, __FILE__, $sql);
   }
...
/*
//isteðe baðlý
//üyenin yaptýðý raporlamalarýda sil
...
//ctracker login historisini de sil

   $sql = "DELETE FROM " . CTRACKER_LOGINHISTORY . "
 ...
 if( isset( $HTTP_POST_VARS['submit'] ) )
 {
  ...
  if ($signature != '')
  {
   ...
   if ( strlen($sig_length_check) > $board_config['max_sig_chars'] && $userdata['user_level'] != ADMIN )
   {
    $error = TRUE;
  ...
  if( !$error )
  {
    ...
    SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) . "', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", $aim) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_sig_bbcode_uid = '$signature_bbcode_uid', user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowavatar = $user_allowavatar, user_allowsig = $user_allowsig, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_allow_pm = $user_allowpm, user_notify_pm = $notifypm, user_popup_pm = $popuppm, user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_active = $user_status, user_can_post = $user_can_post, user_rank = $user_rank" . $avatar_sql . "
    WHERE user_id = $user_id";
    ...
    case USER_AVATAR_UPLOAD:
     ...
     break;
                  case USER_AVATAR_REMOTE:
                   ...
                   {
                   $avatar_url = $this_userdata['user_avatar'];

                   $avatar_status = check_avatar($avatar_url);
                   if ($avatar_status === FALSE)
                   {
                    $avatar = ( $board_config['allow_avatar_remote'] ) ? '<img src="../images/avatar_yok.gif" alt="" border="0" />' : '';
                   }
                   else if ($avatar_status === TRUE)
                   {
                    if ( ($this_userdata['user_avatar_height'] && $this_userdata['user_avatar_height'] > 0) &&
                          ($this_userdata['user_avatar_width'] && $this_userdata['user_avatar_width'] > 0) )
                    {
                      $avatar = '<img src="' . $avatar_url  . '" height="' . $this_userdata['user_avatar_height'] . '" width="' . $this_userdata['user_avatar_width'] . '" alt="" border="0" />';
                    }
                    else  // No width/height in the user's profile
                    {
                     $avatar  = '<img src="' . $avatar_url  . '" alt="" border="0" />';
                    }
                   }
                   }
...
else if ( $mode == 'lookup' )
{
 ...
 if ( !empty($sql_where) )
 {
  ...
  else if ( $db->sql_numrows($result) == 1 )
  {
   ...
   $template->assign_vars(array(
    "META" => '<meta http-equiv="refresh" content="1;url=' . append_sid("admin_users.$phpEx?mode=edit&" . POST_USERS_URL . "=" . $row['user_id']) . '">')
   );
