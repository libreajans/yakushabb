...
function start_edit(val, newname) {
	...
	return overlib('<img src="../images/auth_overall_forum/' + newname + '.gif">', WIDTH, 18, FULLHTML);
}
...
function change_auth(which, baseauth, forumid, type) {
	...
	if (edit_on==true) {
		if (baseauth != edit_cur_name) {
			which.src = "../images/auth_overall_forum/" + baseauth + "_TO_" + edit_cur_name + ".gif";
			inputEl.value = edit_cur_name;
		} else {
			which.src = "../images/auth_overall_forum/" + baseauth + ".gif";
			inputEl.value = '';
		}
...
function start_restore() {
	edit_on = false;
	restore_on = true;
	return overlib('<img src="../images/auth_overall_forum/_restore.gif">', FULLHTML);
}