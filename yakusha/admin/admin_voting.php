...
if( !empty($setmodules) )
	{
		...
		$module['Users']['Admin_voting'] = $filename;
		return;
	}
	...
	// Assign individual vote results
		...
		while ( $row = $db->sql_fetchrow($result) )
			{ 
				...
				if (time() < $vote_end)
					{
						$vote_duration = (date ("d-m-Y",$vote_start)) . " <br> " . (date ("d-m-Y",$vote_end)) . "(Sürüyor)";
					}
				else if ($vote_length == 0)
					{
						$vote_duration = (date ("d-m-Y",$vote_start)) . " <br> " . "Süresiz" ;
					}
				else
					{
						$vote_duration = (date ("d-m-Y",$vote_start)) . " <br> " . (date ("d-m-Y",$vote_end)) . "(Bitti)" ;
					} 
