...
// Generate page
//
$page_title = $lang['usersites'];
$redirect_page = '<input type="hidden" name="redirect" value="'.append_sid("usersites.php?mode=$mode&amp;order=$sort_order").'">';
...
$template->assign_vars(array(
	...
	'S_MODE_ACTION' => append_sid("usersites.$phpEx"))
);
...
if ( $row = $db->sql_fetchrow($result) )
{
	...
	do
	{
		...
		$template->assign_block_vars('memberrow', array(
			...
			'U_VIEWPROFILE' => color_group_colorize_name($user_id, false),
			)
		);
		$i++;
	}
...
if ( $total = $db->sql_fetchrow($result) )
{
	...
	$pagination = generate_pagination("usersites.$phpEx?mode=$mode&amp;order=$sort_order", $total_members_with_website, $board_config['topics_per_page'], $start). '&nbsp;';
}
$db->sql_freeresult($result);
