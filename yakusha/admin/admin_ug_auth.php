  ...
  if ( !empty($sql_where) )
  {
   ...
   else if ( $db->sql_numrows($result) == 1 )
   {
    ...
    $template->assign_vars(array(
     "META" => '<meta http-equiv="refresh" content="1;url=' . append_sid("admin_ug_auth.$phpEx?mode=user&" . POST_USERS_URL . "=" . $row['user_id']) . '">')
    );
...
// @reset($auth_user);
 ...
 $template->assign_vars(array(
  ...
  'S_SWITCH_ACTION' => append_sid("admin_groups.$phpEx"),
  'L_AUTH_LINK_GROUP' => $lang['Go_edit_group'],
  'S_SWITCH_ACTION_GROUP' => append_sid("admin_groups.$phpEx"),
