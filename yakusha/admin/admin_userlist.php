	...
	case 'ban':
		...
		else
		{
			...
			while( $i < count($user_ids) )
			{
				...
				$sql = "INSERT INTO ". BANLIST_TABLE ." (ban_userid, ban_time, ban_by_userid )
					VALUES (". $user_id .", $ban_time, $ban_by_userid )";
				if( !($result = $db->sql_query($sql)) )
				{
                                                ...
                                                if ( $board_config['allow_avatar_remote'] )
                                                  {
                                                    if ( ($row['user_avatar_height'] && $row['user_avatar_height'] > 0) &&
                                                       ($row['user_avatar_width'] && $row['user_avatar_width'] > 0) )
                                                          {
                                                          $avatar_img = '<img src="' . $row['user_avatar'] . '" height="' . $row['user_avatar_height'] . '" width="' . $row['user_avatar_width'] . '" alt="" border="0" />';
                                                          }
                                                          else  // No width/height in the user's profile
                                                            {
                                                              $avatar_img = '<img src="' . $row['user_avatar'] . '" alt="" border="0" />';
                                                            }
                                                  }
