	...
	if ( !empty($session_id) )
	{
		...
		if ( isset($userdata['user_id']) )
		{
			...
			if ($ip_check_s == $ip_check_u)
			{
				...
				//
				if (($current_time - $userdata['session_time'] > 60 || $thispage_id != $userdata['session_page'] ) or $session_force)
				{
...
function append_sid($url, $non_html_amp = false)
{
	...
	global $url_theme;
	if ( !empty($url_theme) && !eregi('theme=', $url) )
	{
		$url .= ( ( strpos($url, '?') != false ) ?  ( ( $non_html_amp ) ? '&' : '&amp;' ) : '?' ) . "theme=" . $url_theme;
	}
