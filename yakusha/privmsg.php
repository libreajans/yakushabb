...
else if ( $mode == 'read' )
{
	...
	if( $HTTP_GET_VARS['view']=='next' || $HTTP_GET_VARS['view']=='prev' )
	{
		...
		$sql_nav = "SELECT pm.privmsgs_id FROM ". PRIVMSGS_TABLE ." pm, ". PRIVMSGS_TABLE ." p2
			WHERE p2.privmsgs_id = $privmsgs_id $pm_sql_user
	...
	$sql = "SELECT u.username AS username_1, u.user_id AS user_id_1, u.user_posts AS user_posts_1, u.user_rank AS user_rank_1, u2.username AS username_2, u2.user_id AS user_id_2, u2.user_posts AS user_posts_2, u2.user_rank AS user_rank_2, u.user_sig_bbcode_uid, u.user_posts, u.user_from, u.user_website, u.user_email, u.user_icq, u.user_aim, u.user_yim, u.user_regdate, u.user_msnm, u.user_viewemail, u.user_rank, u.user_sig, u.user_avatar, pm.*, pmt.privmsgs_bbcode_uid, pmt.privmsgs_text
	...
	$privmsg_subject = ( ( !preg_match('/^'.$lang['Re'].':/', $privmsg['privmsgs_subject']) ) ? $lang['Re'] : '' ) . $privmsg['privmsgs_subject'];
	...
	$template->assign_vars(array(
		...
		'PM_ID' => $privmsg_id,
		'MQR_FORM_ACTION' => append_sid("privmsg.$phpEx"),
	...
	$poster_joined = ( $privmsg['user_id_1'] != ANONYMOUS ) ? $lang['Joined'] . ': ' . create_date('d F Y', $privmsg['user_regdate'], $board_config['board_timezone']) : '';
	$username_from1 = ($privmsg['username_1']);
	$username_from = color_group_colorize_name($privmsg['user_id_1'], false);
	$user_id_from = $privmsg['user_id_1'];
	init_display_pm_attachments($privmsg['privmsgs_attachment']);
	$user_rank_from = $privmsg['user_rank_1'];
	$user_posts_from = $privmsg['user_posts_1'];
	...
	{
		$private_message .= '<br /><img src="images/imza.gif"><br />'.$user_sig;
	}
	...
	if ( $user_rank_from )
	{
		...
		{
			if ( $user_rank_from == $ranksrow[$i]['rank_id'] && $ranksrow[$i]['rank_special'] )
			{
				$rank_from = $ranksrow[$i]['rank_title'];
				$rank_from_image = ( $ranksrow[$i]['rank_image'] ) ? '<img src="' . $images['rank_path'] . $ranksrow[$i]['rank_image'] . '" alt="' . $rank_from . '" title="' . $rank_from . '" border="0" /><br />' : '';
			}
		}
	}
	else
	{
		for($i = 0; $i < count($ranksrow); $i++)
		{
			if ( $user_posts_from >= $ranksrow[$i]['rank_min'] && !$ranksrow[$i]['rank_special'] )
			{
				$rank_from = $ranksrow[$i]['rank_title'];
				$rank_from_image = ( $ranksrow[$i]['rank_image'] ) ? '<img src="' . $images['rank_path'] . $ranksrow[$i]['rank_image'] . '" alt="' . $rank_from . '" title="' . $rank_from . '" border="0" /><br />' : '';
			}
	...
	if ( $user_rank_to )
	{
		for($i = 0; $i < count($ranksrow); $i++)
		{
			if ( $user_rank_to == $ranksrow[$i]['rank_id'] && $ranksrow[$i]['rank_special'] )
			{
				$rank_to = $ranksrow[$i]['rank_title'];
				$rank_to_image = ( $ranksrow[$i]['rank_image'] ) ? '<img src="' . $images['rank_path'] . $ranksrow[$i]['rank_image'] . '" alt="' . $rank_to . '" title="' . $rank_to . '" border="0" />' : '';
			}
		}
	}
	else
	{
		for($i = 0; $i < count($ranksrow); $i++)
		{
			if ( $user_posts_to >= $ranksrow[$i]['rank_min'] && !$ranksrow[$i]['rank_special'] )
			{
				$rank_to = $ranksrow[$i]['rank_title'];
				$rank_to_image = ( $ranksrow[$i]['rank_image'] ) ? '<img src="' . $images['rank_path'] . $ranksrow[$i]['rank_image'] . '" alt="' . $rank_to . '" title="' . $rank_to . '" border="0" />' : '';
			}
	...
	{
		$temp_url = append_sid("search.$phpEx?search_author=" . urlencode($privmsg['username_1']));
		$search_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_search'] . '" alt="' . $lang['Search_u_posts'] . '" title="' . $lang['Search_u_posts'] . '" border="0" /></a>';
		$search = '<a href="' . $temp_url . '" title="' . $lang['Search_u_posts'] . '">' . $lang['Posts'] . ': ' . $privmsg['user_posts'] . '</a>';
	}
	else
	{
		$search_img = '';
	...
	$template->assign_vars(array(
		...
		'RANK_FROM' => $rank_from,
		'RANK_FROM_IMG' => $rank_from_image,
		'RANK_TO' => $rank_to,
		'RANK_TO_IMG' => $rank_to_image,
...
else if ( $submit || $refresh || $mode != '' )
{
	...
	if ( $submit && $mode != 'edit' )
	{
		...
		}// admin fload sýnýrý kaldýr sonu
	}
	...
	if ( $submit && !$error )
	{
		...
		$template->assign_vars(array(
			'META' => '<meta http-equiv="refresh" content="1;url=' . append_sid("privmsg.$phpEx?folder=inbox") . '">')
		);
	...
	else
	{
		...
		else if ( $mode == 'reply' || $mode == 'quote' )
		{
			...
			if ( $mode == 'reply' )
			{
				...
				//$to_usernamed = color_group_colorize_name($to_userid, true);
				$template->assign_block_vars('postrow', array(
					'ROW_COLOR' => '#' . $theme['td_color1'], 
					'ROW_CLASS' => $theme['td_class1'], 

					'MINI_POST_IMG' => $images['icon_minipost'],
					'POSTER_NAME' => $to_usernamed,
					'POST_DATE' => create_date($board_config['default_dateformat'], $privmsg['privmsgs_date'], $board_config['board_timezone']),
...
// View PM while replying MOD, By Manipe
	...
	if ( $preview && !$error )
	{
		...
		{
			$preview_message = $preview_message . '<br /><img src="images/imza.gif"><br>'. str_replace("\n", "<br />", $user_sig);
		}
	...
	$template->assign_vars(array(
		...
		'L_SELECT_ALL' => $lang['Selecet_all'],

		'S_MESSAGE_NO_FOUND' => $lang['Message_no_found'],
		'BBCODE_B_HELP' => $lang['bbcode_b_help'],
		'BBCODE_I_HELP' => $lang['bbcode_i_help'],
		'BBCODE_U_HELP' => $lang['bbcode_u_help'],
		'BBCODE_QUOTE_HELP' => $lang['bbcode_quote_help'],
		'BBCODE_CODE_HELP' => $lang['bbcode_code_help'],
		'BBCODE_IMG_HELP' => $lang['bbcode_img_help'],
		'BBCODE_URL_HELP' => $lang['bbcode_url_help'],
		'BBCODE_FC_HELP' => $lang['bbcode_fc_help'],
		'BBCODE_FS_HELP' => $lang['bbcode_fs_help'],
		'BBCODE_FT_HELP' => $lang['bbcode_ft_help'],
		'BBCODE_MAIL_HELP' => $lang['bbcode_mail_help'],
		'BBCODE_RIGHT_HELP' => $lang['bbcode_right_help'],
		'BBCODE_LEFT_HELP' => $lang['bbcode_left_help'],
		'BBCODE_CENTER_HELP' => $lang['bbcode_center_help'],
		'BBCODE_JUSTIFY_HELP' => $lang['bbcode_justify_help'],
		'BBCODE_WEB_HELP' => $lang['bbcode_web_help'],
		'BBCODE_PLAIN_HELP' => $lang['bbcode_plain_help'],
		'BBCODE_HR_HELP' => $lang['bbcode_hr_help'],
		'BBCODE_LIST_HELP' => $lang['bbcode_list_help'],
		'BBCODE_STRIKE_HELP' => $lang['bbcode_strike_help'],
		'L_SELECT_ALL' => $lang['Selecet_all'],

		'BBCODE_ALT_SOL' => $lang['bbcode_left_alt'],
		'BBCODE_ALT_ORTA' => $lang['bbcode_center_alt'],
		'BBCODE_ALT_SAG' => $lang['bbcode_right_alt'],
		'BBCODE_ALT_JUST' => $lang['bbcode_justify_alt'],
		'BBCODE_ALT_B' => $lang['bbcode_b_alt'],
		'BBCODE_ALT_I' => $lang['bbcode_i_alt'],
		'BBCODE_ALT_U' => $lang['bbcode_u_alt'],
		'BBCODE_ALT_S' => $lang['bbcode_strike_alt'],
		'BBCODE_ALT_CODE' => $lang['bbcode_code_alt'],
		'BBCODE_ALT_QUOTE' => $lang['bbcode_quote_alt'],
		'BBCODE_ALT_LIST' => $lang['bbcode_list_alt'],
		'BBCODE_ALT_URL' => $lang['bbcode_url_alt'],
		'BBCODE_ALT_WEB' => $lang['bbcode_web_alt'],
		'BBCODE_ALT_MAIL' => $lang['bbcode_mail_alt'],
		'BBCODE_ALT_IMG' => $lang['bbcode_img_alt'],
		'BBCODE_ALT_HR' => $lang['bbcode_hr_alt'],
...
if ( $folder != 'outbox' )
{
	...
	{
		case 'inbox':
			@$l_box_size_status = sprintf($lang['Inbox_size'], $inbox_limit_pct, $count_messages);
			break;
		case 'sentbox':
			@$l_box_size_status = sprintf($lang['Sentbox_size'], $inbox_limit_pct, $count_messages);
			break;
		case 'savebox':
			@$l_box_size_status = sprintf($lang['Savebox_size'], $inbox_limit_pct, $count_messages);
			break;
		default:
			@$l_box_size_status = '';
			break;
...
if ( $row = $db->sql_fetchrow($result) )
{
	...
	do
	{
		...
		$template->assign_block_vars('listrow', array(
			'ROW_COLOR' => '#' . $row_color,
			'ROW_CLASS' => $row_class,
			'FROM' => color_group_colorize_name($msg_userid, false),
			'SUBJECT' => $msg_subject,
			'DATE' => $msg_date,
