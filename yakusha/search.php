...
else if ( $search_keywords != '' || $search_author != '' || $search_id )
{
	...
	if ( $search_id == 'egounanswered' || $search_id == 'egotopics' || $search_id == 'usertopics' || $search_id == 'egoreplies' || $search_id == 'newtopics' || $search_id == 'newreplies' || $search_id == 'watched' || $search_id == 'announce' || $search_id == 'newannounce' || $search_id == 'sticky' || $search_id == 'newsticky' || $search_id == 'newposts' || $search_id == 'egosearch' || $search_keywords != '' || $search_author != '' )
	{
		...
		if ( $search_id == 'newposts' || $search_id == 'egosearch' || ( $search_author != '' && $search_keywords == '' ) )
		{
			...
			else
			{
				...
				{
					message_die(GENERAL_ERROR, "Could not obtain list of matching users (searching for: $search_author)", "", __LINE__, __FILE__, $sql);
				}
		...
		//---[+]yakusha ilavesi--------------------------
		//belli bir üyenin baþlýklarýný bul
		...
		else if ( $search_id == 'newtopics' )
		{
			if ( $auth_sql != '' )
			{
				$sql = "SELECT t.topic_id, f.forum_id
					FROM " . TOPICS_TABLE . " t, " . FORUMS_TABLE . " f
					WHERE t.topic_time >= " . $userdata['user_lastvisit'] . "
		...
		else if ( $search_id == 'newreplies' )
		{
			if ( $auth_sql != '' )
			{
				$sql = "SELECT p.topic_id, f.forum_id
		...
		else if ( $search_id == 'watched' )
		{
			if ( $auth_sql != '' )
			{
				$sql = "SELECT t.topic_id, f.forum_id
					FROM " . TOPICS_WATCH_TABLE . " t, " . TOPICS_TABLE . " t2, " . FORUMS_TABLE . " f
					WHERE t2.topic_id = t.topic_id
						AND t.user_id = " . $userdata['user_id'] . "
						AND t2.forum_id = f.forum_id
						AND t2.topic_moved_id = 0
						AND $auth_sql";
			}
			else
			{
				$sql = "SELECT t.topic_id 
					FROM " . TOPICS_WATCH_TABLE . " t, " . TOPICS_TABLE . " t2
					WHERE t2.topic_id = t.topic_id
						AND t.user_id = " . $userdata['user_id'] . "
						AND t2.topic_moved_id = 0";
			}
	...
	if ( $search_results != '' )
	{
		...
		for($i = 0; $i < count($searchset); $i++)
		{
			...
			if ( $show_results == 'posts' )
			{
				...
				$poster = ($searchset[$i]['user_id'] == ANONYMOUS ) ? (($searchset[$i]['post_username'] != '' ) ? $searchset[$i]['post_username'] : $lang['Guest'] ) : color_group_colorize_name($searchset[$i]['user_id'], true);
			...
			else
			{
				...
				$topic_author = ($searchset[$i]['user_id'] == ANONYMOUS ) ? (($searchset[$i]['post_username'] != '' ) ? $searchset[$i]['post_username'] : $lang['Guest'] ) : color_group_colorize_name($searchset[$i]['user_id'], true);
				...
				$last_post_author = ( $searchset[$i]['id2'] == ANONYMOUS ) ? ( ($searchset[$i]['post_username2'] != '' ) ? $searchset[$i]['post_username2'] : $lang['Guest'] ) : color_group_colorize_name($searchset[$i]['id2'], true);
				$last_post_url = '<a href="' . append_sid("viewtopic.$phpEx?" . POST_POST_URL . '=' . $searchset[$i]['topic_last_post_id']) . '#' . $searchset[$i]['topic_last_post_id'] . '">'.$last_post_time.'</a>';
