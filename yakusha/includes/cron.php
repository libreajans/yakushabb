...
function mark_start_finish ( $action )
{
	...
	$sql = "UPDATE `" . BACKUP_TABLE . "` SET " . $update_time . " `finished` = '" . $action . "';";
...
function markLastRun() 
{
	...
	$sql = "UPDATE `" . BACKUP_TABLE . "` SET `last_run` = '" . $time . "';";
...
function write_cache($filename, $data)
{
	...
	if( !@is_dir($phpbb_root_path . '/cache/auto_backup/') )
	{
		if ($debug) echo "Cache sub-directory doesnt exist, attempting to create it\n";
		if( !@mkdir($phpbb_root_path . '/cache/auto_backup/') )
		{
			if ($debug) echo "Can't create cache sub-dir\n";
			return false;
		}
		else
		{
			if ($debug) echo "Directory made! Chmoding to 777\n";
			@chmod($path, 0777);
		}
	}
	
	if ($handle = opendir($phpbb_root_path . '/cache/auto_backup/'))
	{
			if ($debug) echo "Emptying Directory:\n";
			while (false !== ($item = readdir($handle))) 
			{
				if ($item != "." && $item != ".." && $item != "index.htm" ) 
				{
					if ($debug) echo "\t Removing $item from cache\n";
					unlink($phpbb_root_path . '/cache/auto_backup/' . $item);
				}
			}
		closedir($handle);
	}
	...
	fputs($file, "<?php\n\n// Automatic Database Backup Mod Cache.\n// Mod by kkroo (Omar Ramadan) phpbb-login.strangled.net\n// Generated on " . date('r') . " (time=" . time() . ")\n\n \$lastScheduled = $data;\n\n?>");
	fputs($file, $code);
	fclose($file);
	@chmod($filename, 0777);
...
function parseCron() 
{
	...
	$filename = $phpbb_root_path . '/cache/auto_backup/auto_backup_' . md5( $job['lastActual'] . $when ) . '.' . $phpEx;
