...
if ( !empty($setmodules) )
{
	...
	$module['ZDatabase']['ADB_Maintenance'] = $filename;
	return;
}
	...
	case 'perform': // Execute the commands
		...
		switch($function)
		{
			case 'statistic': // Statistics
				...
				if ( !$result )
				{
					throw_error("Could not obtain MySQL Version", __LINE__, __FILE__, $sql);
				}
				...
				$template->assign_vars(array(
					...
					'YAKUSHA_VERSION' => $board_config['yakusha_ver'],
					...
					'L_YAKUSHA_VERSION' => $lang['Yakusha_versiyon'],
			...
			case 'check_user': // Check user tables
				...
				if ( !$result )
				{
					throw_error("Could not get user information!", __LINE__, __FILE__, $sql);
				}
				...
				else // anonymous user does not exist
				{
					...
					if ( !$result )
					{
						throw_error("Could not add user data!", __LINE__, __FILE__, $sql);
					}
				...
				if ( !$result )
				{
					throw_error("Could not update pending information!", __LINE__, __FILE__, $sql);
				}
				...
				if ( !$result )
				{
					throw_error("Could not get user and group data!", __LINE__, __FILE__, $sql);
				}
				...
				if ( count($multiple_groups) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not get group data!", __LINE__, __FILE__, $sql);
					}
					while ( $row = $db->sql_fetchrow($result) )
					{
						$result_array[] = $row['group_id'];
					...
					if ( !$result )
					{
						throw_error("Could not delete groups!", __LINE__, __FILE__, $sql);
					}
				...
				if ( count($missing_groups) )
				{
					...
					for($i = 0; $i < count($missing_groups); $i++)
					{
						...
						if ( !$result )
						{
							throw_error("Could not add group data!", __LINE__, __FILE__, $sql);
						}
						...
						if ( !$result )
						{
							throw_error("Could not add user - group connection!", __LINE__, __FILE__, $sql);
						}
					}
				}
				if (!$db_updated)
				...
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					if ( !$result2 )
					{
						throw_error("Could not update group data!", __LINE__, __FILE__, $sql2);
					}
				}
				$db->sql_freeresult($result);
				if ($list_open)
				...
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					if ( !$result2 )
					{
						throw_error("Could not get group data!", __LINE__, __FILE__, $sql2);
					}
					if ( !($row2 = $db->sql_fetchrow($result2)) ) // No record found
					{
						...
						if ( !$result3 )
						{
							throw_error("Could not insert data in user-group-table!", __LINE__, __FILE__, $sql3);
						}
					}
					elseif ( $row2['user_pending'] == 1 ) // Record found but moderator is pending
					{
						...
						if ( !$result3 )
						{
							throw_error("Could not update data in user-group-table!", __LINE__, __FILE__, $sql3);
						}
					}
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update user-group data!", __LINE__, __FILE__, $sql);
					}
					$affected_rows = $db->sql_affectedrows();
					if ( $affected_rows == 1 )
					{
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update group data!", __LINE__, __FILE__, $sql);
					}
					$affected_rows = $db->sql_affectedrows();
					if ( $affected_rows == 1 )
					{
				...
				if ( !$result )
				{
					throw_error("Could not get user and rank data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update user data!", __LINE__, __FILE__, $sql);
					}
				}
				else
				{
				...
				if ( !$result )
				{
					throw_error("Could not get user and theme data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if ( $row['user_style'] == '' )
					{
						...
						if ( !$result2 )
						{
							throw_error("Could not update themes data!", __LINE__, __FILE__, $sql2);
						}
						$result_array[] = 0;
					}
					else
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not get themes data!", __LINE__, __FILE__, $sql);
					}
					...
					if ( !$result )
					{
						throw_error("Could not update themes data!", __LINE__, __FILE__, $sql);
					}
				}
				else
				{
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not get language data!", __LINE__, __FILE__, $sql);
					}
					if ( $row = $db->sql_fetchrow($result) )
					{
					...
					{
						throw_error("Could not get config data! Please check your configuration table.");
					}
					$db->sql_freeresult($result);
					...
					for($i = 0; $i < count($result_array); $i++)
					{
						...
						if ( !$result )
						{
							throw_error("Could not update language language data!", __LINE__, __FILE__, $sql);
						}
					}
				...
				if ( !$result )
				{
					throw_error("Could not get banlist and user data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update ban data!", __LINE__, __FILE__, $sql);
					}
					$affected_rows = $db->sql_affectedrows();
					if ( $affected_rows == 1 )
					{
				...
				if ( $phpbb_version[0] == 0 && $phpbb_version[1] >= 18 )
				{
					...
					if ( !$result )
					{
						throw_error("Could not get session key data!", __LINE__, __FILE__, $sql);
					}
					while ( $row = $db->sql_fetchrow($result) )
					{
					...
					if ( count($result_array) )
					{
						...
						if ( !$result )
						{
							throw_error("Could not update session key data!", __LINE__, __FILE__, $sql);
						}
						$affected_rows = $db->sql_affectedrows();
						if ( $affected_rows == 1 )
						{
			...
			case 'check_post': // Checks post data
				...
				if ( !$result )
				{
					throw_error("Could not get post and user data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update post information!", __LINE__, __FILE__, $sql);
					}
				}
				else
				{
				...
				if ( !$result )
				{
					throw_error("Could not get topic and user data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
					...
					if ( !$result2 )
					{
						throw_error("Could not update topic information!", __LINE__, __FILE__, $sql2);
					}
				}
				$db->sql_freeresult($result);
				if ($list_open)
				...
				if ( !$result )
				{
					throw_error("Could not get categories and forums data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update forum information!", __LINE__, __FILE__, $sql);
					}
				}
				else
				{
				...
				if ( !$result )
				{
					throw_error("Could not get post data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not delete post data!", __LINE__, __FILE__, $sql);
					}
				...
				if ( !$result )
				{
					throw_error("Could not get topic and post data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not delete topic data!", __LINE__, __FILE__, $sql);
					}
					$update_post_data = TRUE;
				}
				else
				...
				if ( !$result )
				{
					throw_error("Could not get topic and forum data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update topic information!", __LINE__, __FILE__, $sql);
					}
					$sql = "UPDATE " . POSTS_TABLE . "
						SET forum_id = $new_forum
						WHERE topic_id IN ($record_list)";
				...
				if ( !$result )
				{
					throw_error("Could not get post and topic data!", __LINE__, __FILE__, $sql);
				}
				...
				while ( $row || count($result_array) )
				{
					if ( $current_topic != $row['topic_id'] || !$row )
					{
						if ( count($result_array) )
						{
							...
							if ( !$result2 )
							{
								throw_error("Could not get post information!", __LINE__, __FILE__, $sql2);
							}
							$row2 = $db->sql_fetchrow($result2);
							if ( !$row2 )
							{
								throw_error("Could not get post information!");
							}
							$db->sql_freeresult($result2);
							...
							if ( !$result2 )
							{
								throw_error("Could not update topic data!", __LINE__, __FILE__, $sql2);
							}
							...
							if ( !$result2 )
							{
								throw_error("Could not update post information!", __LINE__, __FILE__, $sql2);
							}
						}
						// Reset data
						$result_array = array();
				...
				if ( !$result )
				{
					throw_error("Could not get post and text data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
				...
				if ( !$result )
				{
					throw_error("Could not get topic data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					$result_array[] = $row['topic_id'];
				...
				if ( !$result )
				{
					throw_error("Could not get forum and prune data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					$result_array[] = $row['forum_id'];
				...
				if ( !$result )
				{
					throw_error("Could not get topic-watch and user data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
				...
				if ( !$result )
				{
					throw_error("Could not get topic-watch and topic data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
				...
				if ( count($user_array) || count($topic_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update topic-watch data!", __LINE__, __FILE__, $sql);
					}
					$affected_rows = $db->sql_affectedrows();
					if ( $affected_rows == 1 )
					{
				...
				if ( !$result )
				{
					throw_error("Could not get auth-access and group data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
				...
				if ( !$result )
				{
					throw_error("Could not get auth-access and forum data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
				...
				if ( count($group_array) || count($forum_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update auth-access data!", __LINE__, __FILE__, $sql);
					}
					$affected_rows = $db->sql_affectedrows();
					if ( $affected_rows == 1 )
					{
			...
			case 'check_vote': // Check vote tables
				...
				if ( !$result )
				{
					throw_error("Could not get vote and topic data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not delete vote data!", __LINE__, __FILE__, $sql);
					}
				...
				if ( !$result )
				{
					throw_error("Could not get vote and result data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
				...
				if ( !$result )
				{
					throw_error("Could not get topic and vote data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					$result_array[] = $row['topic_id'];
				...
				if ( !$result )
				{
					throw_error("Could not get vote data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
					...
					if ( !$result2 )
					{
						throw_error("Could not delete vote result data!", __LINE__, __FILE__, $sql2);
					}
				}
				$db->sql_freeresult($result);
				if ($list_open)
				...
				if ( !$result )
				{
					throw_error("Could not get voters and user data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
				...
				if ( !$result )
				{
					throw_error("Could not get voters and vote data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
				...
				if ( count($user_array) || count($vote_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update voters data!", __LINE__, __FILE__, $sql);
					}
					$affected_rows = $db->sql_affectedrows();
					if ( $affected_rows == 1 )
					{
			...
			case 'check_pm': // Check private messages
				...
				if ( !$result )
				{
					throw_error("Could not get private message data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not delete private message data!", __LINE__, __FILE__, $sql);
					}
				}
				else
				{
				...
				if ( !$result )
				{
					throw_error("Could not get private messages and text data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not delete private message text data!", __LINE__, __FILE__, $sql);
					}
					$affected_rows = $db->sql_affectedrows();
					if ( $affected_rows == 1 )
					{
				...
				if ( !$result )
				{
					throw_error("Could not get private message and user data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					$result_array[] = $row['privmsgs_id'];
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update private message information!", __LINE__, __FILE__, $sql);
					}
				}
				else
				{
				...
				if ( !$result )
				{
					throw_error("Could not get user and private message data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					$result_array[] = $row['user_id'];
					if ($row['new_counter'] != $row['user_new_privmsg'] )
					{
						...
						if ( !$result2 )
						{
							throw_error("Could not update user information!", __LINE__, __FILE__, $sql2);
						}
					}
				}
				$db->sql_freeresult($result);
				...
				if ( !$result )
				{
					throw_error("Could not get user data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
			...
			case 'check_config': // Check config table
				...
				while (list($key, $value) = each($default_config))
				{
					...
					if ( !$result )
					{
						throw_error("Could not query config table!", __LINE__, __FILE__, $sql);
					}
					if ( !($row = $db->sql_fetchrow($result)) )
					{
						...
						if ( !$result )
						{
							throw_error("Could not update config table!", __LINE__, __FILE__, $sql);
						}
					}
				}
				if ($list_open)
			...
			case 'check_search_wordmatch': // Check search word match data
				...
				if ( !$result )
				{
					throw_error("Could not get search-match and post data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
				...
				if ( !$result )
				{
					throw_error("Could not get search-match and word data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
				...
				if ( count($post_array) || count($word_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update search-match data!", __LINE__, __FILE__, $sql);
					}
					$affected_rows = $db->sql_affectedrows();
					if ( $affected_rows == 1 )
					{
			...
			case 'check_search_wordlist': // Check search word list data
				...
				if ( !$result )
				{
					throw_error("Could not get search data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					if ( count($result_array) >= 100 )
					{
						...
						if ( !$result2 )
						{
							throw_error("Could not update search words!", __LINE__, __FILE__, $sql2);
						}
						$affected_rows += $db->sql_affectedrows();
						$result_array = array();
					}
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update search words!", __LINE__, __FILE__, $sql);
					}
					$affected_rows += $db->sql_affectedrows();
				}
				if ( $affected_rows == 1 )
			...
			/*case 'rebuild_search_index': // Rebuild Search Index
				...
				if ( !($db->sql_query($sql)) )
				{
					throw_error("Could not delete from search result table!", __LINE__, __FILE__, $sql);
				}
				$sql = "DELETE FROM " . SEARCH_WORD_TABLE;
				if ( !($db->sql_query($sql)) )
				{
					throw_error("Could not delete from search-word table!", __LINE__, __FILE__, $sql);
				}
				$sql = "DELETE FROM " . SEARCH_MATCH_TABLE;
				if ( !($db->sql_query($sql)) )
				{
					throw_error("Could not delete from search-match table!", __LINE__, __FILE__, $sql);
				}
				...
				if ( !($db->sql_query($sql)) )
				{
					throw_error("Could not reset auto_increment!", __LINE__, __FILE__, $sql);
				}
				echo("<p class=\"gen\">" . $lang['Done'] . "</p>\n");
					...
					case 4: // use advanced method if we have PHP 4+ (we can make use of the advanced array functions)
						...
						for ($i = 0; $i < $array_count; $i++)
						{
							...
							if ( !is_null($cache_word_id) && ( $last_post_id <> $result_array[0][$i] || $last_word_id <> $cache_word_id || $last_title_match <> $result_array[1][$i] ) )
							{
								...
								{
									include('./page_header_admin.'.$phpEx);
									throw_error("Could not insert into search match!", __LINE__, __FILE__, $sql);
								}
							}
			...
			*/case 'synchronize_post': // Synchronize post data
			case 'synchronize_post_direct': // Run directly
				...
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					if ( $row2 = $db->sql_fetchrow($result2) )
					{
						...
						if ( !$result3 )
						{
							throw_error("Could not get topic information!", __LINE__, __FILE__, $sql3);
						}
						...
						if ( $row3 )
						{
							...
							if ( !$result3 )
							{
								throw_error("Could not update topic information!", __LINE__, __FILE__, $sql3);
							}
						}
					}
					else
				...
				if ( !$result )
				{
					throw_error("Could not get forum and topic data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
					...
					if ( !$result2 )
					{
						throw_error("Could not update forum information!", __LINE__, __FILE__, $sql2);
					}
				}
				$db->sql_freeresult($result);
				if ($list_open)
				...
				if ( count($result_array) )
				{
					...
					if ( !$result )
					{
						throw_error("Could not update forum data!", __LINE__, __FILE__, $sql);
					}
					$affected_rows = $db->sql_affectedrows();
					if ( $affected_rows == 1 )
					{
				...
				if ( !$result )
				{
					throw_error("Could not get forum and post data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					if (!$list_open)
			...
			case 'synchronize_user': // Synchronize post counter of users
				...
				if ( !$result )
				{
					throw_error("Could not get user and post data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					$result_array[] = $row['user_id'];
			...
			case 'synchronize_mod_state': // Synchronize moderator status
				...
				if ( !$result )
				{
					throw_error("Could not get moderator data!", __LINE__, __FILE__, $sql);
				}
				while ( $row = $db->sql_fetchrow($result) )
				{
					$result_array[] = $row['user_id'];
			...
			case 'reset_date': // Reset dates
				...
				if ( !$result )
				{
					throw_error("Could not update post data!", __LINE__, __FILE__, $sql);
				}
				$affected_rows = $db->sql_affectedrows();
				if ( $affected_rows == 1 )
				{
				...
				if ( !$result )
				{
					throw_error("Could not update private message data!", __LINE__, __FILE__, $sql);
				}
				$affected_rows = $db->sql_affectedrows();
				if ( $affected_rows == 1 )
				{
			...
			case 'reset_sessions': // Reset sessions
				...
				if ( !$result )
				{
					throw_error("Could not delete from session table!", __LINE__, __FILE__, $sql);
				}
				$sql = "DELETE FROM " . SEARCH_TABLE;
				$result = $db->sql_query($sql);
				if ( !$result )
				...
				if ( !$result )
				{
					throw_error("Could not restore session data!", __LINE__, __FILE__, $sql);
				}
				echo("<p class=\"gen\">" . $lang['Done'] . "</p>\n");
			...
			case 'check_db': // Check database
				...
				for($i = 0; $i < count($tables); $i++)
				{
					...
					if ( !$result )
					{
						throw_error("Could not check table!", __LINE__, __FILE__, $sql);
					}
					if ( $row = $db->sql_fetchrow($result) )
					{
			...
			case 'repair_db': // Repair database
				...
				for($i = 0; $i < count($tables); $i++)
				{
					...
					if ( !$result )
					{
						throw_error("Could not repair table!", __LINE__, __FILE__, $sql);
					}
					if ( $row = $db->sql_fetchrow($result) )
					{
						if ($row['Msg_type'] == 'status')
			...
			case 'optimize_db': // Optimize database
				...
				for($i = 0; $i < count($tables); $i++)
				{
					...
					if ( !$result )
					{
						throw_error("Could not optimize table!", __LINE__, __FILE__, $sql);
					}
					if ( $row = $db->sql_fetchrow($result) )
					{
						if ($row['Msg_type'] == 'status')
			...
			case 'heap_convert': // Convert session table to HEAP
				...
				if ( !($result = $db->sql_query($sql)) )
				{
					throw_error("Could not get session data!", __LINE__, __FILE__, $sql);
				}
				if ( !($row = $db->sql_fetchrow($result)) )
				{
					throw_error("Could not get session data!", __LINE__, __FILE__, $sql);
				...
				if ( intval($row['count']) > HEAP_SIZE )
				{
					...
					if ( !$result )
					{
						throw_error("Could not delete session data!", __LINE__, __FILE__, $sql);
					}
				}
				...
				if ( !$result )
				{
					throw_error("Could not convert table!", __LINE__, __FILE__, $sql);
				}
