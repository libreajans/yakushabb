...
$default_config = array(
	...
	'board_disable' => '0',
	'sitename' => 'Yakusha',
	'site_desc' => 'Türk iþi Premod / Türk iþi phpBB',
	...
	'board_email_sig' => 'Ýlginiz için teþekkürler',
	'board_email' => 'eposta@adresiniz.com',
	'smtp_delivery' => '0',
	'smtp_host' => '',
	'smtp_username' => '',
	'smtp_password' => '',
...
function update_config($name, $value)
{
	...
	if( !$result )
	{
		throw_error("Could not update forum configuration!", __LINE__, __FILE__, $sql);
	}
...
function get_table_statistic()
{
	...
	if( !$result )
	{
		throw_error("Could not obtain table data", __LINE__, __FILE__, $sql);
	}
	while( $row = $db->sql_fetchrow($result) )
	{
		$stat['all']['count']++;
...
function create_cat()
{
	...
	if (!$cat_created)
	{
		...
		if( !$result )
		{
			throw_error("Could not get categories data!", __LINE__, __FILE__, $sql);
		}
		$row = $db->sql_fetchrow($result);
		$db->sql_freeresult($result);
		if( !$row )
		...
		if( !$result )
		{
			throw_error("Could not update categories data!", __LINE__, __FILE__, $sql);
		}
...
function create_forum()
{
	...
	if (!$forum_created)
	{
		...
		if( !$result )
		{
			throw_error("Could not get forum data!", __LINE__, __FILE__, $sql);
		}
		$row = $db->sql_fetchrow($result);
		$db->sql_freeresult($result);
		if( !$row )
		...
		if( !$result )
		{
			throw_error("Could not update forums data!", __LINE__, __FILE__, $sql);
		}
...
function create_topic()
{
	...
	if (!$topic_created)
	{
		...
		if( !$result )
		{
			throw_error("Could not update topics data!", __LINE__, __FILE__, $sql);
		}
...
function get_word_id($word)
{
	...
	if ( !$result )
	{
		...
		throw_error("Could not get search word data!", __LINE__, __FILE__, $sql);
	}
	...
	else // Word was not found
	{
		...
		{
			include('./page_header_admin.'.$phpEx);
			throw_error("Could not insert search word data!", __LINE__, __FILE__, $sql);
		}
...
function set_autoincrement($table, $column, $length, $unsigned = TRUE)
{
	...
	if (check_mysql_version())
	{
		...
		if( !$result )
		{
			throw_error("Could not get table status!", __LINE__, __FILE__, $sql2);
		}
		$row = $db->sql_fetchrow($result);
		$db->sql_freeresult($result);
		if( !$row )
		...
		else
		{
			...
			if( !$result )
			{
				throw_error("Could not alter table!", __LINE__, __FILE__, $sql);
			}
		}
	}
...
function get_config_data($option)
{
	...
	if ( !$result )
	{
		erc_throw_error("Could not get config data!", __LINE__, __FILE__, $sql);
	}
	$row = $db->sql_fetchrow($result);
	$db->sql_freeresult($result);
	if ( !$row )
