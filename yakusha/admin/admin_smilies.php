...
$ct_ignorepvar = array('smile_code','smile_emotion');
...
if( isset($HTTP_GET_VARS['import_pack']) || isset($HTTP_POST_VARS['import_pack']) )
{
	...
	if ( !empty($smile_pak) )
	{
		...
		if( !empty($clear_current)  )
		{
			...
			{
				message_die(GENERAL_ERROR, "Could not delete current smilies", "", __LINE__, __FILE__, $sql);
			}
		}
		else
		{
			...
			if( !$result = $db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, "Could not get current smilies", "", __LINE__, __FILE__, $sql);
			}
		...
		if( empty($fcontents) )
		{
			message_die(GENERAL_ERROR, "Could not read smiley pak file", "", __LINE__, __FILE__, $sql);
		}
		...
		for( $i = 0; $i < count($fcontents); $i++ )
		{
			...
			for( $j = 2; $j < count($smile_data); $j++)
			{
				...
				if( $sql != '' )
				{
					...
					if( !$result )
					{
						message_die(GENERAL_ERROR, "Could not update smilies!", "", __LINE__, __FILE__, $sql);
					}
				}
			}
		}
	...
	switch( $mode )
	{
		case 'delete':
			...
			if( $confirm )
			{
				...
				if( !$result )
				{
					message_die(GENERAL_ERROR, "Could not delete smiley", "", __LINE__, __FILE__, $sql);
				}
		...
		case "save":
			...
			if( !($result = $db->sql_query($sql)) )
			{
				message_die(GENERAL_ERROR, "Could not update smilies info", "", __LINE__, __FILE__, $sql);
			}
		...
		case "savenew":
			...
			if( !$result )
			{
				message_die(GENERAL_ERROR, "Could not insert new smiley", "", __LINE__, __FILE__, $sql);
			}
...
else
{
	...
	if( !$result )
	{
		message_die(GENERAL_ERROR, "Could not obtain smileys from database", "", __LINE__, __FILE__, $sql);
	}
