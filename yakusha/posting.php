...
$ct_ignorepvar = array('message','new_edit_note', 'helpbox');
	...
	if ( $mode == 'editpost' && $board_config['edit_notes_enable'] )
	{
		...
		if ( $show_edit_notes )
		{
			...
			while ( $row = $db->sql_fetchrow($result) )
			{
				...
				{
					$sql2 = 'SELECT user_id FROM ' . USERS_TABLE . ' WHERE user_id = ' . $user_id;
					...
					$u_user_profile = color_group_colorize_name($row2['user_id'],true);
				}
...
else if ( $mode == 'vote' )
{
	...
	if ( !empty($HTTP_POST_VARS['vote_id']) )
	{
		...
		$template->assign_vars(array(
			'META' => '<meta http-equiv="refresh" content="1;url=' . append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">')
		);
	...
	switch ( $mode )
	{
		...
		case 'reply':
			...
			if ( $error_msg == '' )
			{
				...
				submit_post($mode, $post_data, $return_message, $return_meta, $forum_id, $topic_id, $post_id, $poll_id, $topic_type, $bbcode_on, $html_on, $smilies_on, $attach_sig, $bbcode_uid, str_replace("\'", "''", $username), str_replace("\'", "''", $subject), str_replace("\'", "''", $message), str_replace("\'", "''", $poll_title), $poll_options, $poll_length, $edit_notes);
			}
		break;
...
if ( $mode == 'newtopic' || ( $mode == 'editpost' && $post_data['first_post'] ) )
{
	...
	if( $is_auth['auth_sticky'] )
	{
		...
		$topic_type_toggle .= ' /> ' . $lang['Topic_Sticky'] . '&nbsp;&nbsp;';
	}
	...
	{	
		...
		$topic_type_toggle .= ' /> ' . $lang['Topic_Announcement'] . '&nbsp;&nbsp;';
	}

	if ( $topic_type_toggle != '' )
	{
		$topic_type_toggle = '<input type="radio" name="topictype" value="' . POST_NORMAL .'"' . ( ( $post_data['topic_type'] == POST_NORMAL || $topic_type == POST_NORMAL ) ? ' checked="checked"' : '' ) . ' /> ' . $lang['Topic_Normal'] . '&nbsp;&nbsp;' . $topic_type_toggle;
	}
}
