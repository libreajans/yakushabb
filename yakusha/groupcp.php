...
function generate_user_info(&$row, $date_format, $group_mod, &$from, &$posts, &$joined, &$poster_avatar, &$profile_img, &$profile, &$search_img, &$search, &$pm_img, &$pm, &$email_img, &$email, &$www_img, &$www, &$icq_status_img, &$icq_img, &$icq, &$aim_img, &$aim, &$msn_img, &$msn, &$yim_img, &$yim)
{
	...
	$www_img = ( $row['user_website'] ) ? '<a href="' . $row['user_website'] . '" ' . add_nofollow($row['user_website']) . ' target="_userwww"><img src="' . $images['icon_www'] . '" alt="' . $lang['Visit_website'] . '" title="' . $lang['Visit_website'] . '" border="0" /></a>' : '';
	$www = ( $row['user_website'] ) ? '<a href="' . $row['user_website'] . '" ' . add_nofollow($row['user_website']) . ' target="_userwww">' . $lang['Visit_website'] . '</a>' : '';
...
if ( isset($HTTP_POST_VARS['groupstatus']) && $group_id )
{
	...
	$template->assign_vars(array(
		'META' => '<meta http-equiv="refresh" content="1;url=' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">')
	);
